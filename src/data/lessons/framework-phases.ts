...
result = await Runner.run(triage, "I was charged twice")`,
  ...
  "crewai-agents-roles": fw({
    concept:
      "A CrewAI Agent is a role: goal, backstory, tools, and LLM. The crew works because roles do not overlap.",
    whyItExists:
      "Multi-agent demos fail when everyone is 'a helpful assistant'. Roles are the API of the team.",
    analogy:
      "A newsroom: reporter, editor, fact-checker. Nobody is 'generic writer'.",
    technicalExplanation:
      "Agent(role, goal, backstory, tools, llm, allow_delegation, memory). Keep tools on the role that should use them. allow_delegation=True only for a manager. Verbose logs during learning; off in prod.",
    example:
      "Researcher (search tool) → Writer (no search) → Editor (no search). Writer cannot silently google.",
    code: `from crewai import Agent

researcher = Agent(
    role="Researcher",
    goal="Find sourced facts",
    backstory="You only record claims with a URL.",
    tools=[search_tool],
    verbose=True,
)
writer = Agent(
    role="Writer",
    goal="Turn facts into a one-page brief",
    backstory="You never invent citations.",
    tools=[],
)`,
    commandsToRemember: [
      "role + goal + backstory",
      "Tools belong to a role",
      "No overlapping jobs",
      "Delegation is for managers",
    ],
    revisionNotes: {
      cheatSheet: ["Roles are contracts", "Split tools", "Manager delegates", "Writer doesn't search"],
    },
    glossary: ["Role", "Backstory", "Delegation"],
    commonMistakes: ["Same backstory on every agent", "Giving every agent every tool"],
    learnElsewhere: ["Tasks & Process", "Build a Crew"],
  }),

  "crewai-tasks-process": fw({
    concept:
      "A Task is a unit of work with expected output, assigned to one agent. Process is sequential or hierarchical.",
    whyItExists:
      "Without tasks, a crew is a group chat. Tasks make output inspectable and order explicit.",
    analogy:
      "A sprint board: ticket, owner, definition of done. Process is kanban vs a tech lead assigning work.",
    technicalExplanation:
      "Task(description, expected_output, agent, context=[prior_task]). Process.sequential runs in list order. Process.hierarchical uses a manager agent to assign. Context passes prior task output forward. Mark expected_output as a schema in words ('markdown with 3 bullets + sources').",
    example:
      "Task 1 research → Task 2 write (context=[t1]) → Task 3 edit (context=[t2]). Sequential. No manager needed.",
    code: `from crewai import Task, Crew, Process

t1 = Task(description="Research topic X", expected_output="5 sourced bullets", agent=researcher)
t2 = Task(description="Write a brief", expected_output="400-word markdown", agent=writer, context=[t1])
crew = Crew(agents=[researcher, writer], tasks=[t1, t2], process=Process.sequential)
result = crew.kickoff()`,
    commandsToRemember: [
      "Task has expected_output",
      "context=[prior] to chain",
      "sequential vs hierarchical",
      "kickoff() runs the crew",
    ],
    revisionNotes: {
      cheatSheet: ["Tasks not vibes", "Chain with context", "Hierarchical needs a manager", "Inspect each output"],
    },
    glossary: ["Task", "Process", "kickoff"],
    commonMistakes: ["One giant task for the whole job", "Hierarchical with no manager agent"],
    learnElsewhere: ["CrewAI Flows", "Multi-agent — Phase 18"],
  }),

  "crewai-flows": fw({
    concept:
      "Flows are CrewAI's event-driven graphs: @start, @listen, state, and routers when a crew is too linear.",
    whyItExists:
      "Crews are great at role pipelines. Flows handle branching, loops, and mixing crews with plain Python.",
    analogy:
      "Crews are a film crew on set. Flows are the shooting schedule with weather delays and pickup days.",
    technicalExplanation:
      "class MyFlow(Flow[MyState]). @start() kicks off. @listen(method) runs after. router() returns the next method name. You can kickoff a Crew inside a flow step. Persist Flow state if the job is long.",
    example:
      "Start → research crew → if sources < 3, search again → write crew → end.",
    code: `from crewai.flow.flow import Flow, listen, start

class ResearchFlow(Flow):
    @start()
    def begin(self):
        return {"topic": "agent evals"}

    @listen(begin)
    def research(self, state):
        out = research_crew.kickoff(inputs=state)
        return {"draft": str(out)}

    @listen(research)
    def write(self, state):
        return write_crew.kickoff(inputs=state)`,
    commandsToRemember: [
      "@start then @listen",
      "Flow state is explicit",
      "Crew.kickoff inside a step",
      "Router for branches",
    ],
    revisionNotes: {
      cheatSheet: ["Flows = branching", "Crews = roles", "Combine them", "Don't flow a 2-step script"],
    },
    glossary: ["Flow", "@listen", "router"],
    commonMistakes: ["Using Flows for a straight 3-task crew", "No state type"],
    learnElsewhere: ["LangGraph — Phase 10 (if you need HITL checkpoints)"],
  }),

  "build-crewai-crew": fw({
    concept:
      "Ship a three-agent sequential crew with real tools on one role, expected outputs, and a kickoff you can rerun.",
    whyItExists:
      "CrewAI interviews ask you to design roles and tasks, not recite the import path.",
    analogy:
      "A table-read: three people, three jobs, one script, then you perform it twice.",
    technicalExplanation:
      "Researcher + search tool, writer, editor. Sequential tasks with context. Inputs dict on kickoff. Log token cost. Cap max_iter. Do not let the writer call search.",
    example:
      "Input topic='MCP vs A2A'. Output: 1-page brief with 5 sources. Editor rejects unsourced claims.",
...
print(crew.kickoff(inputs={"topic": "MCP vs A2A"}))`,
    project: "Three roles, one search tool, sequential tasks, saved markdown output.",
    commandsToRemember: [
      "3 roles, 3 tasks",
      "Tool on researcher only",
      "kickoff(inputs=...)",
      "Read the editor output",
    ],
    revisionNotes: {
      cheatSheet: ["Small crew", "Clear DoD", "Rerun with new topic", "Cost-cap iterations"],
    },
    glossary: ["Crew", "verbose", "inputs"],
    commonMistakes: ["Five agents for a one-page brief", "No expected_output"],
    learnElsewhere: ["Agent Evaluation — Phase 19"],
  }),
